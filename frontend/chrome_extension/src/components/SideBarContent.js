...
const SideBarContent = ({ termResultsFromServer }) => {
  ...
  useEffect(async () => {
    let update = await serverRequest(
      "recentActivityGet",
      "POST",
      null,
      null,
    ...
    if (recentActivityUpdate) {
      ...
      if (recentActivityUpdate.recentPostIndices) {
        ...
        recentPosts.forEach((postDict, index) => {
          ...
          recentPosts[index] = {
            ...postDict,
            ...author.recentBlog.posts[postDict.index],
          };
        });
      }
      let recentRepos;
      if (recentActivityUpdate.recentRepoIndices) {
        ...
        recentRepos.forEach((repoDict, index) => {
          ...
          recentRepos[index] = {
            ...repoDict,
            ...author.recentRepos[repoDict.index],
          };
        });
      }
  ...
  useEffect(() => {
    if (Array.isArray(termResultsFromServer)) {
      let newAddedToWatchlist = new Array(termResultsFromServer.length).fill(
        false
      );
    ...
    if (type == "updateWatchlist") {
      ...
      if (action == "add") {
        ...
        serverRequest(
          type,
          "POST",
          { action: action, authorName: authorName },
          null,
          setError
        );
      }
      ...
      let updatedTerms = await serverRequest(
        type,
        "POST",
        selection,
        null,
      ...
      serverRequest(
        "rateResults",
        "POST",
        { term: termResults[selection].term, rating: false },
        null,
        setError
      );
      setTermResults((termResults) => {
        let newTermResults = termResults.filter(
          (item, index) => index != selection
        );
