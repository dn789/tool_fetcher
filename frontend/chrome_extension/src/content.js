...
function InjectSideBar() {
  ...
  useEffect(() => {
    chrome.runtime.onMessage.addListener(async function fromBackground(
      message,
      sender,
      sendResponse
    ) {
      if (message.type == "run") {
        ...
        if (message.findTerms) {
          ...
          setTermResults(
            await findTerms(
              message.tabInfo.url,
              message.tabInfo.type,
              message.tabInfo.serializedFile,
              setErrorWrapper
            )
          );
        }
  ...
  const setErrorWrapper = (type) => {
    ...
    if (type == "fetch") {
      message =
        "ERROR: Can't connect to server. Ensure server is running and refresh this page.";
    }
    setError(message);
  };
