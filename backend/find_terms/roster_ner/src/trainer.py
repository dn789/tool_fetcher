...
class RoSTERTrainer(object):
    ...
    def __init__(self, args):
        ...
        if args.gradient_accumulation_steps < 1:
            raise ValueError(
                f"Invalid gradient_accumulation_steps parameter: {args.gradient_accumulation_steps}, must be >= 1")

        if args.train_batch_size != 32:
            print(
                f"Batch size for training is {args.train_batch_size}; 32 is recommended!")
            exit(-1)
        ...
        self.noise_train_update_interval = args.noise_train_update_interval * \
            args.gradient_accumulation_steps
        ...
        if args.do_train:
            tensor_data = self.processor.get_tensor(
                dataset_name="train", max_seq_length=self.max_seq_length, drop_o_ratio=0.5)
        ...
        if args.do_eval:
            tensor_data = self.processor.get_tensor(
                dataset_name=args.eval_on, max_seq_length=self.max_seq_length)
    ...
    def prepare_train(self, lr, epochs):
        ...
        num_train_steps = int(len(
            self.train_data) / self.train_batch_size / self.gradient_accumulation_steps) * epochs
    ...
    def noise_robust_train(self, model_idx=0):
        ...
        model, optimizer, scheduler = self.prepare_train(
            lr=self.noise_train_lr, epochs=self.noise_train_epochs)
        train_sampler = RandomSampler(self.train_data)
        train_dataloader = DataLoader(
            self.train_data, sampler=train_sampler, batch_size=self.train_batch_size)
        ...
        for epoch in range(self.noise_train_epochs):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch}")):
                if (i + 1) % self.noise_train_update_interval == 0:
                    ...
                    model.train()
                    print(
                        f"bin_loss: {round(bin_loss_sum/self.noise_train_update_interval,5)}; type_loss: {round(type_loss_sum/self.noise_train_update_interval,5)}")
                    bin_loss_sum = 0
                    type_loss_sum = 0
                ...
                type_loss = loss_fct(
                    type_logits.view(-1, self.num_labels - 1), type_labels.view(-1), type_weights)
                type_loss_sum += type_loss.item()

                bin_loss = loss_fct(bin_logits.view(-1, 1),
                                    bin_labels.view(-1), bin_weights)
        ...
        eval_dataloader = DataLoader(
            self.train_data, sampler=eval_sampler, batch_size=self.eval_batch_size)
        y_pred, pred_probs = self.eval(model, eval_dataloader)
        torch.save({"pred_probs": pred_probs}, os.path.join(
            self.temp_dir, f"y_pred_{model_idx}.pt"))
    ...
    def update_weights(self, model):
        ...
        for batch in tqdm(train_dataloader, desc="Updating training data weights"):
            ...
            with torch.no_grad():
                ...
                type_pred_label = torch.gather(
                    type_pred, dim=-1, index=type_labels.unsqueeze(-1)).squeeze(-1)
                entity_prob = torch.sigmoid(bin_logits)
                bin_pred = torch.cat((1 - entity_prob, entity_prob), dim=-1)
                bin_pred_label = torch.gather(
                    bin_pred, dim=-1, index=bin_labels.unsqueeze(-1)).squeeze(-1)

                condition = type_pred_label > self.tau
                type_weight[:, :max_len][(
                    valid_pos > 0) & type_pos] = condition.to(type_weight)
        ...
        for i in range(1, self.num_labels):
            ...
            remove_type_num = (
                type_label_pos & remove_label_pos.cpu()).sum().item()
    ...
    def ensemble_pred(self, fild_dir):
        ...
        for i in range(len(pred_prob_list[0])):
            ...
            for j in range(len(pred_prob_list[0][i])):
                all_pred_probs = torch.cat([pred_prob_list[k][i][j].unsqueeze(
                    0) for k in range(len(pred_prob_list))], dim=0)
                ensemble_prob_sent.append(torch.mean(
                    all_pred_probs, dim=0, keepdim=True))
            ensemble_probs.append(torch.cat(ensemble_prob_sent, dim=0))
        ensemble_preds = []
        for pred_prob in ensemble_probs:
            preds = pred_prob.argmax(dim=-1)
            ensemble_preds.append(
                [self.inv_label_map[pred.item()] for pred in preds])
        all_valid_pos = self.tensor_data["all_valid_pos"]
        ensemble_label = -100 * \
            torch.ones(all_valid_pos.size(
                0), all_valid_pos.size(1), self.num_labels)
    ...
    def ensemble_train(self):
        ...
        model, optimizer, scheduler = self.prepare_train(
            lr=self.ensemble_train_lr, epochs=self.ensemble_train_epochs)
        ...
        ensemble_train_data = TensorDataset(
            all_input_ids, all_attention_mask, all_valid_pos, all_soft_labels)
        train_sampler = RandomSampler(ensemble_train_data)
        train_dataloader = DataLoader(
            ensemble_train_data, sampler=train_sampler, batch_size=self.train_batch_size)
        ...
        for epoch in range(self.ensemble_train_epochs):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch}")):
                ...
                target_type = target_type / \
                    target_type.sum(dim=-1, keepdim=True)
            ...
            print(
                f"bin_loss: {round(bin_loss_sum/step,5)}; type_loss: {round(type_loss_sum/step,5)}")
    ...
    def aug(self, mask_prob=0.15, save_name="aug.pt"):
        ...
        train_sampler = RandomSampler(self.train_data)
        train_dataloader = DataLoader(
            self.train_data, sampler=train_sampler, batch_size=self.eval_batch_size)
        all_aug_input_ids = []
        all_idx = []
        for batch in tqdm(train_dataloader, desc="Creating augmentations"):
            idx, input_ids, attention_mask, valid_pos, _ = tuple(
                t.to(self.device) for t in batch)
            aug_input_ids = input_ids.clone()

            mask_pos = torch.rand(
                input_ids.size(), device=self.device) < mask_prob
            orig_ids = input_ids[valid_pos > 0]
            input_ids[mask_pos] = self.mask_id
            with torch.no_grad():
                if self.multi_gpu:
                    mlm_logits = model.module.mlm_pred(
                        input_ids, attention_mask, valid_pos)
                else:
                    mlm_logits = model.mlm_pred(
                        input_ids, attention_mask, valid_pos)
                ...
                sampled_ids = top_idx[torch.arange(
                    top_idx.size(0)), sampled_token_idx]
    ...
    def soft_labels(self, model, entity_threshold=0.8):
        ...
        for batch in tqdm(train_dataloader, desc="Computing soft labels"):
            idx, input_ids, attention_mask, valid_pos, labels = tuple(
                t.to(self.device) for t in batch)
            type_distrib = torch.zeros(input_ids.size(
                0), self.max_seq_length, self.num_labels - 1).to(self.device)
        ...
        type_distribution = type_distribution.view(
            -1, type_distribution.size(-1))
        valid_rows = type_distribution.sum(dim=-1) > 0
        weight = type_distribution[valid_rows]**2 / \
            torch.sum(type_distribution[valid_rows], dim=0)
    ...
    def self_train(self):
        ...
        model, optimizer, scheduler = self.prepare_train(
            lr=self.self_train_lr, epochs=self.self_train_epochs)
        ...
        for epoch in range(self.self_train_epochs):
            ...
            aug_train_data = TensorDataset(
                all_idx, all_input_ids, all_aug_input_ids, all_attention_mask, all_valid_pos, all_soft_labels)
            train_sampler = RandomSampler(aug_train_data)
            train_dataloader = DataLoader(
                aug_train_data, sampler=train_sampler, batch_size=self.train_batch_size)
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch}")):
                ...
                idx, input_ids, aug_input_ids, attention_mask, valid_pos, labels = tuple(
                    t.to(self.device) for t in batch)
                ...
                labels = labels[valid_pos > 0]
                bin_labels = 1 - labels[:, 0]
                bin_loss = bin_loss_fct(
                    bin_logits.view(-1), bin_labels.float())
                bin_loss_sum += bin_loss.item()

                aug_logits, _, = model(
                    aug_input_ids, attention_mask, valid_pos)
    ...
    def performance_report(self, y_true, y_pred):
        for i in range(len(y_true)):
            if len(y_true[i]) > len(y_pred[i]):
                print(
                    f"Warning: Sequence {i} is truncated for eval! ({len(y_pred[i])}/{len(y_true[i])})")
                y_pred[i] = y_pred[i] + ['O'] * \
                    (len(y_true[i]) - len(y_pred[i]))
    ...
    def save_model(self, model, model_name, save_dir):
        ...
        json.dump(model_config, open(os.path.join(
            save_dir, "model_config.json"), "w"))

    # load model from directory
    def load_model(self, model_name, load_dir):
        self.model.load_state_dict(torch.load(
            os.path.join(load_dir, model_name)))
